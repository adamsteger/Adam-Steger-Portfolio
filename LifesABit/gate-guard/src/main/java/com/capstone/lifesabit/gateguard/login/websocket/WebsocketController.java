...
public class WebsocketController {
  ...
  // Map of session ID to session key (GateGuard login session)
  public static HashMap<String, UUID> websocketIDsToLogins = new HashMap<>();
  // Map of session key (GateGuard login session) to websocket session ID
  ...
  public void onOpen(Session session) throws IOException {
    ...
    for (String cookie : cookies) {
      if (cookie.startsWith(LoginController.SESSION_KEY_COOKIE)) {
