...
public class AdminController {
    ...
    ResponseEntity<LoadUsersResponse> loadUsersHandler(HttpServletRequest request, @RequestBody LoadUsersRequest inputs) {
        ...
        if (request.getHeader(LoginController.SESSION_KEY_COOKIE) == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(resp);
        }
        String sessionKey = request.getHeader(LoginController.SESSION_KEY_COOKIE);
        Session adminSession = SessionManager.getSession(UUID.fromString(sessionKey));
        if (!SessionManager.isAuthenticated(sessionKey)) {
            resp.message = "Error: Invalid session key";
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(resp);
        }
        ArrayList<Member> memberList = SQLLinker.getInstance().getMembers();
        if (memberList == null) {
